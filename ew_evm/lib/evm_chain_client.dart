...
import 'package:ew_evm/pending_evm_chain_transaction.dart';
...
abstract class EVMChainClient {
  ...
  Future<bool> connect(Node node) async {
    try {
      ...
      _client = Web3Client(nodeUri.toString(), httpClient);

      if (_client == null) {
        throw Exception("Connection failed, _client is null");
      }

      if ((await _client!.getBlockNumber()) <= 0) {
