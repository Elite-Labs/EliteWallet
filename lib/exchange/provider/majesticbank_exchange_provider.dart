...
class MajesticBankExchangeProvider extends ExchangeProvider {
  ...
  Future<Trade> _createTradeInternal({
    ...
    final headers = <String, String>{
      'from_currency': normalizeCryptoCurrency(request.fromCurrency), 
      'receive_currency': normalizeCryptoCurrency(request.toCurrency),
      'receive_address': request.toAddress,
      'referral_code': referralCode
    };

    if (isFixedRateMode && request.isFixedRate) {
      headers["receive_amount"] = request.toAmount;
    } else {
      headers["from_amount"] = request.fromAmount;
    }
