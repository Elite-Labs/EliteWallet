...
class XMRTOExchangeProvider extends ExchangeProvider {
  XMRTOExchangeProvider(this.settingsStore)
      : _isAvailable = false,
        super(pairList: [
          ExchangePair(
  ...
  static const userAgent = 'EliteWallet/XMR iOS';
  ...
  static Future<bool> _checkIsAvailable(SettingsStore settingsStore) async {
    const url = originalApiUri + _orderParameterUriSuffix;
    final response = await get(settingsStore, url, headers: _headers);
  ...
  Future<bool> checkIsAvailable() async {
    _isAvailable = await _checkIsAvailable(settingsStore);
    return isAvailable;
  }
  ...
  Future<Trade> createTrade({TradeRequest request, bool isFixedRateMode}) async {
    ...
    final response =
        await post(
          settingsStore, url, headers: _headers, body: json.encode(body));
