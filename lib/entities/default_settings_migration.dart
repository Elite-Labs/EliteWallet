...
const wowneroDefaultNodeUri = 'node.suchwow.xyz:34568';
const ethereumDefaultNodeUri = 'ethereum.publicnode.com';
const polygonDefaultNodeUri = 'polygon-bor.publicnode.com';
const eliteWalletBitcoinCashDefaultNodeUri = 'bitcoincash.stackwallet.com:50002';
...
Node? getBitcoinDefaultElectrumServer({required Box<Node> nodes}) {
  return nodes.values
          .firstWhereOrNull((Node node) => node.uriRaw == eliteWalletBitcoinElectrumUri) ??
...
Node? getLitecoinDefaultElectrumServer({required Box<Node> nodes}) {
  return nodes.values
          .firstWhereOrNull((Node node) => node.uriRaw == eliteWalletLitecoinElectrumUri) ??
...
Node? getBitcoinCashDefaultElectrumServer({required Box<Node> nodes}) {
  return nodes.values
          .firstWhereOrNull((Node node) => node.uriRaw == eliteWalletBitcoinCashDefaultNodeUri) ??
...
Future<void> insecureStorageMigration({
  ...
  int? selectedElite2FAPreset = sharedPreferences.getInt(SecureKey.selectedElite2FAPreset);
  ...
  try {
    ...
    await secureStorage.write(
        key: SecureKey.selectedElite2FAPreset, value: selectedElite2FAPreset.toString());
...
Future<void> changeDefaultMoneroNode(
  ...
  final newEliteWalletNode = Node(uri: newEliteWalletMoneroUri, type: WalletType.monero);

  await nodeSource.add(newEliteWalletNode);

  if (needToReplaceCurrentMoneroNode) {
    await sharedPreferences.setInt(PreferencesKey.currentNodeIdKey, newEliteWalletNode.key as int);
  }
}
...
Future<void> checkCurrentNodes(
  ...
  if (currentBitcoinElectrumServer == null) {
    final eliteWalletElectrum = Node(uri: eliteWalletBitcoinElectrumUri, type: WalletType.bitcoin);
    await nodeSource.add(eliteWalletElectrum);
    await sharedPreferences.setInt(
        PreferencesKey.currentBitcoinElectrumSererIdKey, eliteWalletElectrum.key as int);
  }

  if (currentLitecoinElectrumServer == null) {
    final eliteWalletElectrum = Node(uri: eliteWalletLitecoinElectrumUri, type: WalletType.litecoin);
    await nodeSource.add(eliteWalletElectrum);
    await sharedPreferences.setInt(
        PreferencesKey.currentLitecoinElectrumSererIdKey, eliteWalletElectrum.key as int);
  }
  ...
  if (currentBitcoinCashNodeServer == null) {
    final node = Node(uri: eliteWalletBitcoinCashDefaultNodeUri, type: WalletType.bitcoinCash);
    await nodeSource.add(node);
...
Future<void> resetBitcoinElectrumServer(
  ...
  var eliteWalletNode = nodeSource.values
      .firstWhereOrNull((node) => node.uriRaw.toString() == eliteWalletBitcoinElectrumUri);

  if (eliteWalletNode == null) {
    eliteWalletNode = Node(uri: eliteWalletBitcoinElectrumUri, type: WalletType.bitcoin);
    await nodeSource.add(eliteWalletNode);
  }

  if (currentElectrumSeverId == oldElectrumServer?.key) {
    await sharedPreferences.setInt(
        PreferencesKey.currentBitcoinElectrumSererIdKey, eliteWalletNode.key as int);
  }
