...
import 'package:elite_wallet/core/wallet_connect/eth_transaction_model.dart';
...
import 'package:elite_wallet/core/wallet_connect/models/connection_model.dart';
import 'package:elite_wallet/src/screens/wallet_connect/widgets/connection_widget.dart';
import 'package:elite_wallet/src/screens/wallet_connect/widgets/modals/web3_request_modal.dart';
import 'package:elite_wallet/src/screens/wallet_connect/utils/string_parsing.dart';
...
class EvmChainServiceImpl implements ChainService {
  ...
  Future<Web3Client> getEthClient() async {
    ...
    Uri nodeUri =
      appStore.settingsStore.getCurrentNode(appStore.wallet!.type).uri;
    ...
    ethClient = Web3Client(nodeUri.toString(), http.Client());
    return ethClient!;
  }
  ...
  Future<String> ethSignTransaction(String topic, dynamic parameters) async {
    ...
    try {
      final result = await (await getEthClient()).sendTransaction(
        credentials,
        transaction,
        chainId: getChainIdBasedOnWalletType(appStore.wallet!.type),
      );
